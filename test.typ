...
// 表格调用实例:
// #custom_table(
//   title: [实验结果对比omg this is a test],
//   cols: (1fr, 1fr),
//   alns: "cc",
//   "方法",
//   "准确率",
...
虚拟性,并发性,异步性,共享性,持久性
=== 虚拟性
包含内存虚拟性和CPU虚拟性.前者在计组第五章讲过,我们利用页表来执行内存虚拟化,这样程序只用关心如何在OS给定的那个_大的、连续的_虚拟地址里如何运作即可.

后者是:通过“快如闪电”的轮换让多个程序看起来像是在同时运行.程序的错觉:每个程序都觉得自己一直霸占着 CPU,从未被中断过;用户的错觉: 你觉得所有程序都在“并行”运行.
=== 并发性
看CPU虚拟化.
=== 异步性
非常自然.OS会打断你的程序,让你的程序等待;所以程序的执行时间是_不可预期的_.
=== 共享性
互斥&锁.
=== 持久性
这里的意思是文件系统能够持久地保存信息.
...
#img("assets/image.png", width: 70%, caption: [
  执行环境一览,感觉很自然.
])
